...
namespace IntroEvent4
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            user1.VideoPublishing(new MediaFile { FileName = "Полезная мысль #1" });
            user1.Followers += user2.Alert;
            user1.Followers += user3.Alert;
            user1.Followers += user4.Alert;
            user1.Followers += user5.Alert;

            user1.VideoPublishing(new MediaFile { FileName = "Полезная мысль #2" });
            user1.VideoPublishing(new MediaFile { FileName = "Полезная мысль #3" });

            WriteLine();
            user2.Followers += user2.Alert;
            user2.VideoPublishing(new MediaFile { FileName = "Полезная история #1" });
